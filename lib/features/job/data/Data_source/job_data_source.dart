...
class JobDataSource {
  ...
  getJobOwnerID() async {
    ...
    QuerySnapshot jobOwnerSnap = await FirebaseFirestore.instance
        ...
        .get();
    for (var element in jobOwnerSnap.docs) {
  ...
  getWorderID() async {
    ...
    QuerySnapshot workerSnap = await FirebaseFirestore.instance
        ...
        .get();
    for (var element in workerSnap.docs) {
      var data = element.data() as Map<String, dynamic>;
  ...
  getMaxSalary() async {
    QuerySnapshot jobQuery = await firebaseFirestore
        ...
        .limit(1)
        .get();
    JobModels job = JobModels();
    for (var element in jobQuery.docs) {
      var data = element.data() as Map<String, dynamic>;
      job = JobModels.fromJson(data, element.id);
    }
    return job;
  }
  ...
  searchJob(
    ...
    if (salary == null) {
      JobModels minJob = await getMinSalary();
      salary = [];
      salary.add(minJob.salary);

      JobModels maxJob = await getMaxSalary();
    ...
    QuerySnapshot querySnap = await firebaseFirestore
        ...
        .where(jobcloseCategory, isEqualTo: "*")
        ...
        .get();
    List<JobModels> jobsList = [];
    for (var element in querySnap.docs) {
      var data = element.data() as Map<String, dynamic>;

      jobsList.add(JobModels.fromJson(data, element.id));
    }
    return jobsList;
  }
  ...
  getLastMessage(workerId, companyId) {
    firebaseFirestore
        .collection(messagesCollection)
        .where(workerMessage, isEqualTo: workerId)
